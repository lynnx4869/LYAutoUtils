...
public class LYAutoSwitchs: UIView, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, CAAnimationDelegate {
    ...
    public var curIndex: Int = 0 {
        willSet {
            ...
            anim.toValue = [newCell.center.x, bounds.size.height-1.5]
            anim.duration = 0.5
            anim.isRemovedOnCompletion = false
            anim.fillMode = .forwards
            selectLine.layer.add(anim, forKey: "moving")
            
            selectPosition = CGPoint(x: newCell.center.x, y: bounds.size.height-1.5)
        }
    }
    
    public func animationDidStop(_ anim: CAAnimation, finished flag: Bool) {
        selectLine.layer.position = selectPosition
        selectLine.layer.removeAllAnimations()
    }
    ...
    private var selectPosition: CGPoint = .zero
